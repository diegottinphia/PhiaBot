 /*
* PhiaBot es una creación de Diegöttinphia
* PhiaBot no tiene ningun fin de lucro
* Phia se reserva todos los derechos de autor
* © 2021 Phiaautomotive, INC.
...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:Phia\n' // Nombre
            + 'ORG:Phia;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=12029844612:+1 (202) 984-4612\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
...
/******FIN DE ARCHIVOS ANTILINK POR Phia******/
...
function addMetadata(packname, author) {	
	if (!packname) packname = 'PhiaBot'; if (!author) author = 'phia';	
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `CIAO!! @${num.split('@')[0]} ¿COME STAI?😃\n\n『Benvenuto A *${mdata.subject}*』\n\nPiacere di conoscerti amic@ 🤗\n\n_Ricordati di leggere il regolamento del gruppo per non avere problemi 🧐_\n\n*Goditi questo gruppo e divertiti 🥳*`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `Ebenne se n´è andato @${num.split('@')[0]} 👋\n\nChe Dio lo benedica 😎`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '12029844612@s.whatsapp.net'
...
//FUNCION ANTILINK
	...
	if (budy.includes("https://s.kwai.app/")){
		...
		reply(`*LINK DI CAZZO RILEVATO 📢* ${sender.split("@")[0]} Verrai rimosso da questo gruppo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
			client.updatePresence(from, Presence.composing)
			reply("Ciao amico mio")
		}, 0)
	}
	...
	if (budy.includes("https://www.facebook.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DI FACEBOOK RILEVATO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://youtu.be/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DI YOUTUBE RILEVATO📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://www.instagram.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DI INSTAGRAM RILEVATO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
	...
	if (budy.includes("https://vm.tiktok.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DI TIK TOK DI MERDA RILEVATO 📢* ${sender.split("@")[0]} Usted sera eliminado de este grupo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
       ...
       if (budy.includes("://chat.whatsapp.com/")){
		...
		client.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`*LINK DI WHATSAPP RILEVATO 📢* ${sender.split("@")[0]} Usted será expulsado del grupo`)
		setTimeout( () => {
			client.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 0)
		setTimeout( () => {
...
/******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'PhiaBot'; if (!author) author = 'Diegöttinphia';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
...
/******JUEGOS PHIA LA PUTA MADRE NO TE OLVIDES******/					
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'creador':
	       client.sendMessage(from, {displayname: "SPhia 🥸", vcard: vcard}, MessageType.contact, { quoted: mek})
		client.sendMessage(from, 'Arriba está el número del creador del bot <phiaвσт ву Diegöttinphia>\n\nNO SOY UN BOT LPM 🥸\n\nAhi puedes resolver tus preguntas y errores :)\n\nEste no es el numero del propietario del bot que estas usando ahora mismo. Si no, del creador de la base de datos del bot o sea Phia\n\nву Diegöttinphia',MessageType.text, { quoted: mek} )
                const none = fs.readFileSync('./mp3/phia.mp3');
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido recibido, ciao schifo 👋 :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
		     ...
		     if (!isGroupAdmins) return reply(mess.only.admin)
	             if (!isGroupAdmins) return reply(mess.only.Badmin)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, 'CHI VEDIAMO SCHIFO ✋🥸🤚', text, { quoted: mek })
		           if (stdout) {
			...
			//ANTILINKS DE REDES SOCIALES	
				...
				//FIN DE ANTILINK HECHO POR PHIA
